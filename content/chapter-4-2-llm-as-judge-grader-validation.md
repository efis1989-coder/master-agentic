...
*Options:* Raw agreement · Cohen's kappa · Matthews correlation coefficient (MCC) · Position bias · Verbosity bias · Self-preference bias · Sycophancy bias · Style-over-substance bias · Pointwise grading · Pairwise grading · Chain-of-thought grading · Seeded-error detection · Structural independence · Rubric decomposition · Frozen-judge policy

*Check:* Each item below has one defensible answer from the Options list. Verify your protocol addresses each of these before moving to the open design half.

| Item | Answer | Why |
|---|---|---|
| The primary metric that should gate judge certification, because it is robust to class-imbalanced calibration sets | Matthews correlation coefficient (MCC) | The chapter states MCC is "more robust still on imbalanced classes and is the metric that belongs in the gate"; raw agreement inflates on dominant classes and kappa is weaker under severe imbalance |
| The bias that explains why a judge built on the same base model as the agent shares the agent's blind spots and grades its own house style as excellent | Self-preference bias | §2.3 names this explicitly: "a judge favors outputs from its own model family" — it is the bias that structural independence is designed to prevent |
| The bias responsible for the failure story's 92%-reported vs 71%-true gap, where long assertive summaries outscored short correctly-hedged ones | Verbosity bias | §2.3 and edge-case row 6 both identify verbosity/confidence bias as the root cause: "longer answers score higher … the agent learns to pad, and the judge rewards the padding" |
| The technique that makes judge reasoning auditable by requiring it to state its reasoning before its score | Chain-of-thought grading | §2.2: "chain-of-thought grading — requiring the judge to state its reasoning before its score — improves reliability and … makes the judge auditable, because you can read why it scored as it did" |
| The production monitoring mechanism that catches judge drift before any eval number has visibly moved | Seeded-error detection | §2.5: "when a judge's detection rate on seeded errors falls, the judge has drifted … that is your earliest warning that the instrument needs recalibration" |
| The design principle requiring the grading system to not share failure modes, incentives, or model lineage with the system it grades | Structural independence | §2.5 names this explicitly as the principle the failure story "violated most deeply" |

*Sample solution:* A complete certification protocol for the contract-review agent judge treats the judge as a controlled instrument with a defined entry gate, ongoing health checks, and explicit revocation criteria. A strong protocol addresses the following.

- **Calibration set.** Assemble 300-500 cases labeled by at least two qualified legal reviewers (one senior associate and one junior associate per case), with disagreements adjudicated by a third. Stratify the set to match the production distribution: roughly 30% outputs that are clearly acceptable, 30% clearly unacceptable, and 40% subtly-wrong (the regime where biases matter most). Include deliberate seeds of each bias type — short-correct vs long-wrong pairs, hedged-correct vs assertive-wrong pairs. Refresh the set quarterly or any time the input contract distribution shifts materially.
- **Agreement thresholds.** Gate on MCC, not raw agreement. For a release gate on a liability-sensitive system a floor of MCC ≥ 0.70 and kappa ≥ 0.65 is a reasonable starting point; these numbers should be argued back to the tolerable false-negative rate on missed risks, not picked arbitrarily. At class distributions where roughly equal good and bad outputs exist, kappa and MCC converge — if the set is imbalanced, MCC is the tie-breaker. Both metrics must be computed with confidence intervals; a point estimate on a 300-case set carries a meaningful error bar.
- **Bias battery.** Run five controlled sub-tests before certification: (1) *Position bias* — run all pairwise pairs in both orderings, require verdict-flip rate ≤ 10%; (2) *Verbosity bias* — inject 50 short-correct vs long-wrong pairs seeded into the calibration set, require the judge to rank the short-correct output higher ≥ 85% of the time; (3) *Self-preference bias* — compare judge scores on outputs from its own model family vs a different model family matched for quality on ground truth, require the score gap ≤ 5 points; (4) *Sycophancy bias* — inject 50 pairs where the only difference is assertive vs hedged tone (content otherwise equal), require no significant preference for assertive tone; (5) *Style-over-substance bias* — inject 30 polished-but-wrong vs plain-but-correct pairs, require the judge to rank the plain-correct output higher ≥ 80% of the time. A judge that fails any one sub-test is rejected and redesigned; it does not proceed to the calibration-set gate.
- **Drift monitor.** In production, re-run the seeded-error battery on a rolling weekly basis against the frozen certified judge. Track the seeded-error detection rate as a first-class metric. If detection rate drops ≥ 10 percentage points from the certified baseline, or if a quarterly human re-audit of a 100-case stratified sample shows judge-human MCC has fallen below the certification floor, recalibration is triggered automatically. Any judge-model upgrade by the provider is treated as a new instrument requiring full re-certification before it re-enters the gate.
- **Revocation criteria.** A certified judge immediately reverts to advisory-only (and may not gate releases) if: (a) the drift monitor triggers a recalibration event and the event has not yet been resolved; (b) a human audit reveals a judge-human agreement gap of more than 10 percentage points versus the certified baseline; (c) the judge model version is upgraded by the provider before re-certification is complete; (d) evidence emerges that the generator has learned to Goodhart the rubric (judge scores rising while independent human scores are flat or falling). Advisory mode means scores are logged and reviewed but do not block a release; a human reviewer signs off manually until recertification passes.
