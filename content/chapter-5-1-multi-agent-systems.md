...
Two coordination substrates exist. **Message passing** hands explicit payloads between agents; a shared memory or blackboard lets agents read and write a common store. Message passing with typed contracts is easier to reason about and to audit, because every handoff is a discrete, loggable event. **Shared blackboard**s are powerful but leak coupling — an agent's behavior now depends on what some other agent wrote, and your trace (Ch. 4.3) has to reconstruct a shared-state history, not just a call tree. **Result compression up the hierarchy is not an optimization; it is the mechanism that keeps the orchestrator's context from becoming the union of every worker's noise.**
...
*Options:* Single agent · Multi-agent (parallelizable breadth) · Multi-agent (context partitioning) · Multi-agent (privilege separation)

*Check:* For each task, identify the best-justified architecture decision using the three legitimate motivations from §2.2 and the chapter's strong-default rule.

| Item | Answer | Why |
|---|---|---|
| (a) Large codebase migration to a new framework | Single agent | Migration work is sequential, context-dependent, and error-correlated across files; no mechanical fan-out, context ceiling, or privilege boundary justifies splitting — the organizational feel of "a big project" is not a mechanical reason. |
| (b) Market-landscape scan across a hundred companies | Multi-agent (parallelizable breadth) | One hundred independent company lookups fan out with no cross-dependency; parallel sub-agents cut wall-clock time and each runs in an isolated context window — the canonical case where multi-agent pays. |
| (c) Invoice-dispute resolution workflow | Single agent | Dispute resolution is sequential reasoning over a single context (invoice, contract, communications); it can require privileged write actions but a single agent with scoped tools is the counterfactual to beat, and no genuine fan-out or context-partition need exists. |

*Sample solution:* A thorough answer treats the three tasks in turn, names the default, and designs the accepted case end-to-end.

- **(a) Codebase migration — Single agent.** No legitimate motivation applies. Files must be migrated in dependency order, so there is no independent fan-out for parallelizable breadth. The context needed (the existing codebase, the migration rules, the emerging new state) is continuous, not partitionable into clean rooms. No part requires a separate privilege grant that a single agent with carefully scoped tool permissions cannot handle. Counterfactual: one agent with a code-read tool, a code-write tool, and a test-runner tool, working file-by-file in topological order.
- **(b) Market scan — Multi-agent (parallelizable breadth).** One hundred independent company profiles have no cross-dependency; the wall-clock win of running forty workers in parallel is real, and each worker's isolated context window prevents noise accumulation. This is the one case that passes the test.
  - *Worker contract:* Input schema — `{ company_id: string, research_questions: string[] }`; output schema — `{ company_id: string, findings: Finding[], confidence: "high"|"medium"|"low", sources: string[] }`; isolation boundary — each worker gets only its company slice and a set of search/retrieval tools; no cross-worker reads.
  - *Global budget authority:* Hard ceiling of, for example, 50 000 tokens and 5 tool calls per worker, enforced by the orchestrator before dispatch; a total task ceiling of 2M tokens and 100 worker slots, held above the tree and not negotiable by workers. The orchestrator aborts and surfaces partial results if either ceiling is hit.
  - *Aggregation eval:* A structurally independent verifier (different model or deterministic rule engine) spot-checks 10% of individual findings against their cited sources before the composite report is assembled. The composite is not trusted because workers agree; it is trusted because spot-checked findings are verifiable. A known-incorrect test company can be injected to confirm the verifier catches errors the workers confidently endorse.
- **(c) Invoice-dispute resolution — Single agent.** Dispute resolution requires reading the invoice, the contract, and the communications together — a single coherent reasoning context, not a partitionable one. The workflow may eventually need a write action (issue a credit, update a ledger), but that is a scoped tool grant to one agent, not a privilege-separation argument for two. Counterfactual: one agent with read tools for invoice/contract/communications, a write tool for issuing credits gated by an approval schema, and a human-in-the-loop escalation trigger.
