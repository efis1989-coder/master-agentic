...
*Options:* Positive evidence of correctness · Absence of complaint · Explicit user correction · Human verification sample · Lineage tags · Holdout population · Flywheel-internal metrics · Kill criteria measured against holdout

*Check:* Match each pipeline component to the approach the chapter argues is required.

| Item | Answer | Why |
|---|---|---|
| Qualification criterion for a positive training example | Positive evidence of correctness | The chapter defines a trace as an unexamined example unless there is positive evidence it was right — not merely that nothing bad was logged. |
| Primary source of the highest-quality training signal | Explicit user correction | Section 2.3 calls a correction (the edit the user actually made) the strongest and cleanest signal, explicitly contrasting it with the non-complaint proxy. |
| Who must review a fraction of harvested traces | Human verification sample | Section 2.3 specifies a human verification sample as the gate that tells right from merely-unchallenged — automated signals alone cannot substitute. |
| What must every training example carry to prevent eval contamination | Lineage tags | Section 2.5 names per-example lineage tracking as the only thing standing between you and a benchmark accidentally trained on. |
| What population must be walled off from training, optimization, and evaluation-as-target | Holdout population | Sections 2.5 and 3 require one uncontaminated reference the flywheel never trains on, optimizes toward, or uses as an eval target. |
| What must kill criteria be measured against to remain meaningful | Kill criteria measured against holdout | The Review standard states that kill criteria measured against flywheel-internal metrics cannot detect drift; only the holdout provides an uncontaminated reference. |

*Sample solution:* A compliant pipeline for the support-agent flywheel covers six layers plus an application to the prorated-refund case.

- **Qualification criteria.** A trace is eligible as a positive training example only on positive evidence of correctness: the customer's issue was resolved *and* a human reviewer confirmed the answer was factually correct, *or* the answer was checkable (e.g., matched a policy document) and that check passed, *or* the user explicitly accepted an answer after reviewing it with no correction. "No complaint" alone is never sufficient — it is a proxy for the absence of friction, not a signal of truth.

- **Verification-sampling rate and reviewers.** Sample at least 5–10 % of harvested "successful" traces for human review before they enter the training pool. Reviewers must be domain-knowledgeable (in this case, billing-policy-trained support staff), not general-purpose annotators. The rate should rise for any trace class that has been escalated or corrected at above-baseline frequency.

- **Counterfactual capture.** Record every user correction as a first-class event: the field the user overwrote, the draft they edited before sending, the agent answer they flagged as wrong. A correction is a paired (bad output, user-preferred output) example — the strongest signal the flywheel can ingest. Instrument the product surface to emit these events explicitly (Ch. 4.3 discipline); do not infer them from downstream outcomes.

- **Lineage tags.** Every example carries: source conversation ID, harvest timestamp, qualification method (human-verified / correction-capture / policy-check), reviewer ID if human-verified, and a `used_for` set that accumulates every use — train, eval, optimization target. The pipeline enforces a hard constraint: no example may appear in both train and eval sets, and no eval example may become an optimization target.

- **Holdout policy.** Randomly wall off 5 % of incoming production traffic at initial deployment and freeze that population permanently. The flywheel never trains on it, never optimizes toward it, and never uses it as a live evaluation target. It is evaluated on a fixed quarterly schedule by a separate process, providing the single uncontaminated measure of whether the system is genuinely improving or simply fitting its own history.

- **Kill criteria.** Pause the flywheel if: (a) holdout accuracy on any tracked topic class drops more than 2 percentage points across two consecutive quarterly evaluations; (b) the human-verification sample's correctness rate falls below the baseline set at launch; (c) catastrophic-forgetting regression suite shows degradation on capabilities outside the tuned task. All thresholds are measured against the holdout or the external human sample — never against flywheel-internal metrics.

- **Application — prorated-refund case.** The refund conversations would have been blocked at the qualification gate: "customer didn't reopen the ticket" is a non-complaint proxy, not positive evidence that the refund calculation was correct. The verification sample would have caught the subtly wrong explanation when a billing-knowledgeable reviewer checked it against the actual refund policy. Even if a few slipped past sampling, lineage tracking would have flagged them as unverified, keeping them out of the confirmed-positive pool. With those gates in place, thousands of wrong-but-unchallenged refund traces would never have reached the training corpus, and the fine-tune would not have frozen the house style of the mistake into weights.
