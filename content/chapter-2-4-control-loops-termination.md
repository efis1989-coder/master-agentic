...
**Termination is a property you design into the loop with explicit budgets, gates, and detectors; an agent that can only stop when it decides it is done cannot be trusted to stop at all, because the hardest inputs are exactly the ones that defeat that judgment.** The controls are concrete. *Max turns* and *token/cost budgets* are hard ceilings. *Confidence gates* stop when evidence is sufficient. *Progress metrics* stop when state stops changing. *Deadlock and oscillation detectors* catch the ping-pong before the invoice does. None of these depend on the model volunteering to quit.
