...
Design, on paper and to review standard, a complete agentic platform for a regulated workflow. Your deliverables are: a discovery and specification dossier — the task inventory built from evidence of work-as-done, the expert-elicitation plan, and a gradeable definition of done per sub-task (Ch. 0.3); a task and autonomy analysis (Ch. 0.1, 3.3); a full architecture with an explicit deterministic/agentic seam (Ch. 3.1–3.2); a containment design and threat model (Ch. 3.4–3.5); an evaluation and grader-validation program (Ch. 4.1–4.2); an observability and reliability specification (Ch. 4.3–4.4); a unit-economics model (Ch. 4.5, 5.7); a release and audit posture (Ch. 4.6–4.7); a trust-ladder product specification (Ch. 3.6); where you use third-party RL-trained components, a vendor model-training diligence memo (Ch. 5.5); and a fleet-governance posture for the platform's second year — the agent registry, the paved-road services, and the kill criteria that will govern the agents this platform breeds (Ch. 5.8). Everything is scored against the six-dimension rubric in Section 9 of the syllabus, reproduced at the end of this document.
