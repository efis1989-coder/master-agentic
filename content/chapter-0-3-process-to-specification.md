...
**An agent is only as correct as the specification it is built from; a spec copied from an aspirational process automates a fiction, so the real work is eliciting what experts actually do and encoding that judgment as the definition of done.**
...
*Options:* System traces first · Observation first · Documentation first · Interviews first · Equal weight all sources

*Check:*

| Item | Answer | Why |
|---|---|---|
| Top-tier evidence source for subrogation discovery | System traces first | The chapter ranks system traces highest because they record work-as-done at scale, exposing which exceptions fire weekly before any human can minimise or misremember them. |
| Second-tier evidence source after traces | Observation first | Shadowing a specialist on live file reviews surfaces the core-system workarounds and tacit pre-checks that traces record as outcomes but cannot explain causally. |
| Lowest-trust evidence source for task discovery | Documentation first | The 12-page SOP is work-as-imagined — potentially four years stale — so it must be used last, as a hypothesis checklist tested against mined traces, never as a ground truth. |

*Sample solution:* A model answer for self-comparison. Ground your response in the five-layer specification stack, the work-as-done/work-as-imagined distinction, and all three definition-of-done levels.

- **Discovery plan (priority order).** (1) System traces — mine the ~30% of closed files specialists already reviewed; extract which attributes they acted on, which files were escalated, and what the actual recovery rate was per attribute cluster. Even 270 files from three months yields real path frequencies no SOP can give you. (2) Observation — shadow one specialist on five live file reviews end-to-end; watch the pre-check on the core system for the loss-date quirk analog (every workflow has one). (3) Documentation and interviews — treat the 12-page SOP as a hypothesis checklist: for each SOP step, count how often the traces show it followed; every gap is an interview target, not a ground truth.

- **Elicitation protocol.** Exception-first questions: "Tell me about the last file you pulled from the queue that looked like a recovery but turned out not to be — what killed it?" / "What would a smart new paralegal get confidently wrong in month two?" / "When did you last disagree with each other, and how did it resolve?" Contrastive-pair design: take five matched pairs from the mined traces — same loss type, similar paid amount, opposite recovery outcome — and ask each specialist independently to explain the difference; the boundary articulations are the decision logic. Disagreement detection with n=2: run each specialist through the same 20 cases independently before any joint session, score agreement, and bring every split to a named business owner (recovery manager or head of subrogation) for adjudication into an explicit rule or an escalation condition; do not average the two positions.

- **Definition of done — "assess recovery potential on one closed file."** Hard constraints: (a) all three required system fields (loss date, fault attribution code, liable-party identifier) are present and non-null in the output; (b) the cited policy clause exists verbatim in the policy record; (c) the computed recoverable amount reconciles within $1 to the paid-loss figure in the claims system. Graded qualities: (d) the recovery-potential band (High / Medium / Low / None) matches the band a senior specialist assigns to the same file on independent review — rubric anchor cases are the adjudicated pairs from elicitation; (e) the rationale paragraph cites only facts present in the file and names the liable-party theory of recovery. Escalation conditions: (f) liable-party identity is contested or unknown; (g) loss-date discrepancy between policy and adjuster notes exceeds 30 days; (h) any output field fails a hard constraint.

- **Labeled seed set.** Size: 300 files minimum, because the escalation strata are rare and the verifier must be calibrated before the first eval run (§2.2). Stratification: 40% files the specialists reviewed and recovered, 25% files reviewed and passed, 20% files not reviewed (random from the 70% backlog — this surfaces the missed-recovery rate), 15% deliberate edge cases from the exception-first interviews. Adjudication: the recovery manager resolves every inter-specialist split into a label plus a one-sentence rule; any file neither specialist can confidently label becomes an escalation-class exemplar.

- **Portfolio-triage argument.** Value at stake is high: seven-figure annual missed recoveries across 900 files/month is genuine. Autonomy feasibility is moderate: the output is an assessment and a flag, not a payment or legal filing, so blast radius on a wrong High rating is a wasted specialist hour, not a compliance event. Verifiability is the problem: recovery potential is a leading indicator — actual recovery (the ground truth) closes months later and depends on third-party cooperation. At intake, the only affordable proxy verifier is specialist agreement, which the elicitation phase buys you. This means the eval suite will measure surrogate accuracy (specialist-agreement rate) rather than true accuracy (dollars recovered) for at least 6–12 months. That delay does not disqualify the task — the surrogate is defensible and the value is large — but it should appear in the triage scorecard as a caveat, and the first agent should be scoped to flag files for specialist review (autonomous assessment, human action) rather than autonomous recovery initiation.
