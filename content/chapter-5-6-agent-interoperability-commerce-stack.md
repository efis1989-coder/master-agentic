...
*Options:* Adopt · Pilot · Wait and track

*Check:* Match each stack layer to the correct protocol posture for this bank pilot.

| Item | Answer | Why |
|---|---|---|
| Tool layer (MCP) posture | Adopt | MCP is the most mature and stable layer; §2.5 explicitly lists it as "stable enough to adopt." |
| Agent layer (A2A) posture | Pilot | A2A is standardizing but not yet settled; §2.5 prescribes piloting on "real but bounded use cases." |
| Payment layer posture | Wait and track | The payment layer is "the fastest-moving and least settled"; §2.5 says the posture for most enterprises is to "wait and track" while running small, contained experiments. |

*Sample solution:* A strong bank pilot posture covers five design decisions.

- **Protocol selection per layer.** Tool layer: adopt MCP for all agent-to-software integrations with both vendors — it is stable and the safe foundation. Agent layer: run a bounded A2A pilot with vendor discovery and task delegation on a single, non-critical procurement category; maintain a protocol-version-negotiation handshake and a graceful-abort rule for version skew. Payment layer: wait-and-track — no live payment protocol commits yet; instrument two small experiments behind a manual-approval gate to build institutional knowledge without treasury exposure.
- **Mandate policy.** Every spend or binding commitment requires a signed mandate chain: human signs intent (category, ceiling, acceptable terms) → agent builds cart → payment executed only inside those signed parameters. Ceiling tiers: routine SaaS < $5 k requires manager signature; $5 k–$50 k requires finance controller signature; above $50 k is blocked from agent execution and escalates to a human purchase order. Every mandate is stored as an immutable audit record (Ch. 4.7), providing non-repudiable evidence for any future dispute.
- **Identity-verification requirements.** Counterparty agents must present a signed A2A Agent Card with a verifiable organizational identity. Card validation is a necessary gate — but explicitly not sufficient (§2.3: identity is not trustworthiness).
- **Counterparty eval demands.** Behavioral evidence is required before any live transaction. Vendors must supply: a set of disclosed benchmark tasks and their pass rates; reference customers willing to attest to behavioral quality; and results from at least one adversarial/red-team scenario. The bank retains the right to run its own shadow evaluations (Ch. 4.2) before moving from pilot to production. A signed card with no behavioral attestation is treated as "identity confirmed, trust unestablished."
- **Escalation rules.** Out-of-scope actions are a protocol-level human trigger, not an app-level afterthought. Triggers: any proposed action outside the signed mandate parameters (upsell, modified tier, unfamiliar pricing model), any counterparty response that fails a schema/scope check, any version-skew or authentication failure. On trigger: the agent halts the transaction, logs a structured escalation event, and routes to a named human approver within a defined SLA — the transaction does not resume until a human re-issues or rejects the mandate.
- **Standing protocol-tracking commitment.** A named technology-strategy function (even one person, part-time) reviews the A2A specification changelog and the leading payment-protocol working groups monthly. The signal to move from "wait" to "adopt" on the payment layer: (a) at least one payment standard reaches a stable, versioned release under credible open governance, AND (b) at least two large counterparty banks announce production adoption, AND (c) the bank's own legal and compliance team confirms the mandate structure satisfies local regulatory audit requirements.
