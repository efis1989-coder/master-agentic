...
**An agent's behavior is the joint output of four independently-changeable artifacts — prompt, model, tools, and judge — so unless all four are versioned into one coupled changelog, a metric drop leaves you unable to roll back with confidence or attribute the cause, and every regression becomes a guess rather than a lookup.**
...
*Options:* Prompt only · Prompt + model · Prompt + model + tool schema · Prompt + model + tool schema + judge

*Check:* Each item below has one answer that follows directly from the chapter's doctrine. Pick the single best match.

| Item | Answer | Why |
|---|---|---|
| The complete set of artifacts that must be versioned into one coupled release changelog | Prompt + model + tool schema + judge | The chapter names all four as independently-changeable behavior vectors; leaving any one out recreates the three-suspect attribution failure. |
| The artifact scope that must be tested as a unit in shadow deployment to expose behavior surprises from a combined prompt-and-model upgrade | Prompt + model | Shadow must score the new combination against live traffic; scoring each artifact separately cannot reveal an interaction that only emerges from the two artifacts running together. |
| The minimum set of artifacts that must be coupled and tested together to catch a prompt×model interaction at the regression-CI rung | Prompt + model | Testing each artifact in isolation is exactly the gap that let the failure story's interaction pass offline evals; the CI must run the combination. |
| The scope that must be recorded in the single changelog so that any regression can be attributed and rolled back without guessing | Prompt + model + tool schema + judge | Attribution is only possible when every artifact that can move behavior — all four — appears in one ledger; a partial changelog leaves at least one change vector invisible. |

*Sample solution:* A complete release checklist and auto-rollback spec for a revenue-critical prompt + model upgrade should cover the following:

- **Coupled release unit.** Cut a single release record with four fields: `prompt_version`, `model_version`, `tool_schema_version`, `judge_version`. Add `rationale` (why this change now), `owner` (accountable person), and `timestamp`. Commit this record to one changelog before any artifact enters the gate ladder. This is the fix that would have made the failure story a ten-minute bisect rather than a week-long guess — the Sunday tool registration and the Saturday silent model update would each appear as logged entries with owners, not invisible background noise.

- **Gate ladder.**
  - *Regression CI:* Run the full eval suite from Ch. 4.1 on the *combination* (new prompt × new model), not each in isolation. Gate on the statistical threshold from that chapter — block only when the delta exceeds the minimum detectable effect, so noise does not false-block and real drops do not pass. Target suite runtime under 15 minutes so engineers have no incentive to bypass.
  - *Shadow:* Route 100% of live traffic to the new version in score-only mode (no user-visible response) for a minimum of 48–72 hours, or until the distribution of request types stabilizes. Measure task-completion rate, tool-call patterns, and any sentinel cases that are known to be sensitive to model behavior. The failure story's interaction would have surfaced here — the shadow would have scored the new combination on the real Friday/weekend traffic mix and flagged the drop before canary.
  - *Canary:* Serve the new version to 2% of real traffic for 24 hours, then 10% for 24 hours. Automatic rollback triggers (see below) are armed throughout. Only promote to full rollout if no trigger fires and the canary metrics match shadow expectations.
  - *A/B:* For any change touching memory, trust, or cumulative behavior, hold a full A/B test with a measurement window matched to the change mechanism — days for a phrasing tweak, weeks for anything affecting user trust or retention (per §2.4).

- **Auto-rollback triggers.** Wire the canary deployment to revert automatically — no human in the loop — if any of the following fire within a 10-minute rolling window: task-resolution rate drops more than 3% relative to the control slice (calibrated to the MDE so noise does not trigger), error rate rises more than 1.5× baseline, or any sentinel case produces an outcome outside the acceptable range defined in the regression suite. Thresholds are chosen to be tighter than the failure story's 12% drop so a rollback fires long before the regression reaches statistical certainty on the full population.

- **Re-baselining step.** Before running any gate, re-score the v46 (or current production) release under the *new judge version* if the judge has changed. Record the re-scored baseline as `baseline_v46_judge_v{n}`. All gate comparisons use this re-scored baseline. Never compare a new score to a baseline produced by a different judge. This ensures the before/after delta is real and not an artifact of measurement drift — the category error §2.5 calls "a ruler whose markings shifted mid-measurement."

- **How this catches the failure story.** The prompt×model interaction would have been blocked at shadow: shadow runs the new prompt on the new model against real traffic, sees the combined behavior, and scores it. The canary gate would have been a second catch: 2% of users, auto-rollback armed, so the regression is bounded to a tiny slice and reverted within minutes of the threshold crossing. The key fix is coupling the release — with a single changelog entry recording both the prompt bump and the model pin, the gate runs the *combination* from the start, not each artifact in isolation.
