...
*Options:* Read-financial-records · Process-inbound-email · Send-email · Create-ticket · Quarantined-reader pattern · Per-task derived identity + egress allow-list · Private data · Untrusted content · Egress channel

*Check:* For each item below, identify the correct answer from the options.

| Item | Answer | Why |
|---|---|---|
| Trifecta leg: the tool that supplies private data | Read-financial-records | Financial records are the sensitive data asset; the chapter names read-financial-records as the private-data leg of the finance-ops trifecta |
| Trifecta leg: the tool that supplies untrusted content | Process-inbound-email | Inbound emails are authored by outsiders and are the primary injection surface the chapter identifies |
| Trifecta leg: the tool that supplies the egress channel in the read-records + process-email + send-email triple | Send-email | The chapter names send-email as the egress channel that completes the fully armed trifecta |
| Architectural change that breaks the trifecta on every untrusted-content path at once | Quarantined-reader pattern | Splits the agent so the component reading untrusted content has no tools and no private-data access, breaking the trifecta at the source |
| Architectural change that bounds identity scope and closes the exfiltration destination | Per-task derived identity + egress allow-list | Replaces the broad service account with human-principal-derived scoped credentials and restricts egress to an allow-list, breaking the private-data and egress legs |

*Sample solution:* A complete threat-model of the finance-ops agent should cover all five items as follows.

- **Item 1 — Injection surfaces.** Every content channel the agent reads that an outsider can influence: (1) inbound emails — text carrier, authored by external senders; (2) documents retrieved from a shared knowledge base — document carrier, writable by vendors or external parties; (3) ticket bodies submitted by external users — text carrier; (4) tool results from MCP servers the team does not control — structured-data carrier but still untrusted; (5) filenames and metadata of any uploaded or fetched files — filename/metadata carrier. Each is an injection surface regardless of format.

- **Item 2 — Trifecta analysis by tool combination.** Walk every pairing and triple: *read-financial-records + process-inbound-email + send-email* is a fully armed trifecta — private data (financial records), untrusted content (inbound email), egress channel (send-email). *Read-financial-records + process-inbound-email + create-ticket* is also armed if external users can read their own ticket bodies — egress channel is the ticket visible to the requester. *Process-inbound-email alone* (no private data, no egress beyond the ticket) is lower risk but still a surface. *Read-financial-records + create-ticket* (internal-only queue) has weak egress; if the ticket queue is internal-only, this is the mildest pairing. Every combination that lights up all three legs is a priority for an architectural fix.

- **Item 3 — Two architectural changes.** First, apply the quarantined-reader pattern: split off a model that reads inbound emails and retrieved documents but holds no tools and no private-data access; it returns minimized structured output (e.g., "billing question, negative sentiment") to a privileged planner that never sees the raw content and therefore never sees injected instructions. This breaks the untrusted-content leg for every injection surface at once. Second, replace the super-agent service account with per-task identity derived from the human principal (OAuth on-behalf-of), scoped to that principal's rights, with a lifetime measured in minutes; add an egress allow-list restricting send-email and URL-fetching tools to approved destinations. This breaks both the private-data leg (scope is now one rep's rights, not god-mode) and the egress leg (no novel exfiltration destinations).

- **Item 4 — Identity design.** Each task runs as the support rep or finance analyst who initiated it, not as a shared service account. The credential is an OAuth on-behalf-of token minted at task start, carrying the initiating principal's CRM/finance read scope plus the specific action scope needed (create-ticket or send-email), with a TTL of 10–15 minutes. The token is presented to every downstream system so actions are attributable to a named human and enforceable by that human's own ACLs. No standing key is distributed to the agent; the token is ephemeral and cannot be reused after expiry.

- **Item 5 — Residual risk and monitoring.** After the two architectural changes, residual risks include: (a) injection via non-text carriers (images, PDF metadata, filenames) that the quarantined reader might still process — mitigate with aggressive input normalization and unicode stripping before any model sees the content; (b) memory or tool poisoning if the privileged planner reads from persistent memory that was previously written by an injected session — mitigate by signing memory writes with provenance and auditing them; (c) cross-agent attacks in any multi-agent configuration where the privileged planner receives input from another agent that was itself injected. Monitoring: trace every tool call and correlate its parameters against the preceding untrusted-content ingestion to detect anomalous spillover; alert on any egress attempt to a non-allow-listed host; audit memory writes for provenance; log every token issuance with the initiating principal's ID for attribution.
